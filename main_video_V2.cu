...
// ============================================================================
// KERNEL MODIFICATO PER RICERCA A GRIGLIA
// ============================================================================
...
// ============================================================================
// FUNZIONE PRINCIPALE MODIFICATA CON ROI E GRIGLIA
// ============================================================================
...
int main()
{
    ...
    SearchConfig config;
    config.roiX = 100;           // Inizio ROI
    config.roiY = 200;
    config.roiWidth = 800;       // Dimensioni ROI
    config.roiHeight = 600;
    config.gridStepX = templ.cols;    // Passo griglia = dimensioni template
    config.gridStepY = templ.rows;
    config.useGrid = true;            // Usa ricerca a griglia
    config.threshold = 10000.0f;      // Soglia per match validi
    ...
    while (true) {
        ...
        for (const auto& match : matches) {
            if (match.isValid) {
                cv::Rect detectionRect(match.position, cv::Size(templ.cols, templ.rows));
                cv::rectangle(frame, detectionRect, cv::Scalar(0, 255, 0), 2);
                
                std::string info = "B:" + std::to_string(match.bluePixels) + 
                                 " S:" + std::to_string((int)match.score);
                cv::putText(frame, info, 
                           cv::Point(match.position.x, match.position.y - 10), 
                           cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 255, 0), 1);
                
                printf("  Match: (%d,%d) Score:%.0f Blu:%d\n", 
                       match.position.x, match.position.y, match.score, match.bluePixels);
            }
        }
        
        // Disegna ROI per visualizzazione
        cv::Rect roiRect(config.roiX, config.roiY, config.roiWidth, config.roiHeight);
        cv::rectangle(frame, roiRect, cv::Scalar(255, 0, 0), 1);
        ...
        if (frameCount % 1 == 0) {
            ...
            printf("FPS: %.2f, Frame: %d, Match trovati: %zu\n", 
                   fps, frameCount, matches.size());
        }
        
        // Mostra frame
        cv::imshow("Template Matching con ROI e Griglia", frame);
