    ...
    cv::Point *bestLocSeq)
{
    ...
    auto start_intgralImage = high_resolution_clock::now();

    cv::Mat seqIntegralSqImage;
    computeIntegralImagesSequential(imageN, seqIntegralSqImage);

    auto end_integralImage = high_resolution_clock::now();
    auto duration_integralImage = duration_cast<milliseconds>(end_integralImage - start_intgralImage);
    std::cout << "Tempo calcolo immagine integrale (ms): " << duration_integralImage.count() << std::endl;

    // Creazione dell'immagine integrale dei quadrati del template
    auto start_intgralTemplate = high_resolution_clock::now();
    ...
    auto end_integralTemplate = high_resolution_clock::now();
    auto duration_integralTemplate = duration_cast<milliseconds>(end_integralTemplate - start_intgralTemplate);
    std::cout << "Tempo calcolo immagine integrale template (ms): " << duration_integralTemplate.count() << std::endl;

    // Calcolo matrice di cross correlazione
    auto start_crossC = high_resolution_clock::now();

    cv::Mat crossCorrelation;
    crossCorrelation = crossCorrelationFFT(imageN, templN);

    auto end_crossC = high_resolution_clock::now();
    auto duration_crossC = duration_cast<milliseconds>(end_crossC - start_crossC);
    std::cout << "Tempo calcolo matrice di cross correlazione (ms): " << duration_crossC.count() << std::endl;
    ...
    auto end_SSD = high_resolution_clock::now();
    auto duration_SSD = duration_cast<milliseconds>(end_SSD - start_SSD);
    std::cout << "Tempo calcolo matrice SSD (ms): " << duration_SSD.count() << std::endl;
