...
__device__ float getRegionSum(const float *sumTable, int width, int x, int y, int kx, int ky) {
    int x1 = x - 1;
    int y1 = y - 1;
    int x2 = x + kx - 1;
    int y2 = y + ky - 1;
...
cudaError_t templateMatchingSSD(
    ...
    *bestLoc = minLoc;
    ...
    printf("STAMPE:\n");
    printf("Immagine:\n");
    stampMta(imageFloat,  imageFloat.rows,  imageFloat.cols);
    printf("Immagine Integrale:\n");
    stampMta(cudaIntegralImage, cudaIntegralImage.rows, cudaIntegralImage.cols);
    printf("Immagine Quadrata Integrale:\n");
    stampMta(cudaIntegralSqImage, cudaIntegralSqImage.rows, cudaIntegralSqImage.cols);
    printf("Template:\n");
    stampMta(templFloat, templFloat.rows, templFloat.cols);
    printf("Template Integral value:\n");
    printf("%.2f\n", templateSum);
    printf("Template Squared Integral value:\n");
    printf("%.2f\n", templateSqSum);
    printf("SSD Result:\n", templateSqSum);
